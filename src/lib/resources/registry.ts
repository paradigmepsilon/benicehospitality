...
export const RESOURCE_TOOLS: Record<string, ResourceToolMeta> = {
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // FLEET (Car Rental Riches). Insertion order IS the running order on
  // /resources and every fleet section, because liveResourceTools() walks
  // Object.values(). These ten are sequenced the same way the co-living block
  // is, by where an operator hits them: underwrite the car and the plan, budget
  // the launch, work the 90 days, list it, then run the trip, the guest, and the
  // upkeep, and finally read the fleet and time the exit. A new tool belongs at
  // its stage, NOT appended to the end of the block.
  //
  // Turo plan figures in these tools trace to the CRR fact base (verified
  ...
  "fleet-guest-message-templates": {
    ...
    blurb:
      "The ten Turo trip-lifecycle messages, inquiry through review ask, each on its own screen with a send time, three lengths to pick from, and your guest, car, and pickup filled in.",
    bullets: [
      "All 10 lifecycle messages, one screen each, in trip order",
      "Brief, Standard, or Detailed wording on every message",
      "A recommended send time on each, and why that timing",
      "Type guest, car, and pickup once; every message updates live",
    ...
    subhead:
      "Reviews are won in the messages: the on-time reminder, the clear check-in, the calm fix when something breaks. Ten field-tested messages cover the whole trip, three fields personalize all of them at once, and each one comes in the length that sounds like you.",
    howItWorks: [
      "Enter the guest name, car nickname, and pickup location once.",
      "Open the message for the moment you are in and check its send time.",
      "Pick Brief, Standard, or Detailed, then copy it and send.",
    ],
    whatYouGet: [
      "Ten messages covering the full trip lifecycle, one screen each",
      "Three interchangeable lengths per message, same facts in all three",
      "A recommended send time and the reason behind it",
      "Live personalization across every message at once",
