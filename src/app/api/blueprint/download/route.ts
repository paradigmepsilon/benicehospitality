...
/**
 * GET /api/blueprint/download?format=<pdf|epub>&t=<token>
 *
...
export async function GET(request: Request) {
  ...
  if (!isBlueprintFormat(format)) {
    return NextResponse.json({ error: "Unknown format." }, { status: 400 });
  }
  if (!token || !verifyBlueprintToken(token, format)) {
  ...
  try {
    ...
    console.error("[blueprint/download] signing failed:", err);
    return new NextResponse(
