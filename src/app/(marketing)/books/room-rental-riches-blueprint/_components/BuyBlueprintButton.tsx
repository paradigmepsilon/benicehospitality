...
/**
 * BuyBlueprintButton — POSTs to /api/blueprint/checkout and hands off to the
 * returned Stripe hosted-checkout URL. Same contract as the Host's Edge
 * BuyButton: handles loading + error, and never crashes the page when the API
 * is not configured yet (503 with a plain message).
 */
export default function BuyBlueprintButton({
  label,
  className = "",
  source,
}: {
  ...
  async function handleClick() {
    setLoading(true);
    setError(null);
    posthog.capture("blueprint_checkout_started", { source });
    try {
      const res = await fetch("/api/blueprint/checkout", { method: "POST" });
