...
import type { DocAudience, FleetDoc } from "@/lib/fleet/journey";
import type { DocReview } from "@/lib/fleet/doc-reviews";
import { AUDIENCE_BADGE } from "@/components/admin/partnership/ui";
import { Modal } from "@/components/admin/partnership/Modal";

type LibraryDoc = FleetDoc & { rendered: boolean; editable: string | null };
...
export default function FleetDocsPage() {
  ...
  const fetchAll = useCallback(
    () =>
      Promise.all([
        fetch("/api/admin/fleet/docs").then((res) => (res.ok ? res.json() : [])),
        fetch("/api/admin/fleet/docs/reviews").then((res) => (res.ok ? res.json() : [])),
  ...
  async function toggle(docKey: string, reviewed: boolean) {
    ...
    const res = await fetch("/api/admin/fleet/docs/reviews", {
      method: "POST",
    ...
    <div className="space-y-4">
      ...
      <div className={`rounded-lg border p-4 ${allApproved ? "bg-[#5b9a2f]/10 border-[#5b9a2f]/35" : "bg-white border-[#e8e4dd]"}`}>
        ...
        <p className="mt-2 text-xs text-[#1a1a1a]/55">
          Ticking a box means you read it, not that a lawyer has. The agreement stays a draft until the attorney reviews it, whatever this tracker says.
        </p>
      </div>
        ...
        <>
          {missing > 0 && (
            <p className="text-xs text-[#8a6215]">
              ...
              <code className="bg-[#1a1a1a]/5 px-1 rounded">bash docs/fleet-management/render.sh</code>.
            </p>
          )}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1 text-xs">
                            ...
                            {d.editable && d.audience !== "client" && (
                              <a href={`/api/admin/fleet/files/${d.editable}`} target="_blank" rel="noopener noreferrer" className="font-medium text-[#5b9a2f] hover:underline">
                                Fill in + print ↗
                              </a>
                            )}
                            {review?.approvedAt ? (
      ...
      {confirmApprove && (
        <Modal title="Approve these documents for use?" onClose={() => setConfirmApprove(false)}>
          ...
          <p className="text-sm text-[#1a1a1a]/70 mt-2">
            This does not replace the attorney&apos;s review of the agreement, and it does not confirm the Exhibit B numbers. Those stay blank or marked PROPOSED until Alex confirms them.
          </p>
